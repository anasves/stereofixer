...
class PandasStereoChecker():
    ...
    def __init__(self, projectname: str, no_h_smiles: bool = True, canonicalize_rxns: bool = True):
        """
    ...
    def check_all_reactions(self, reaction_smiles_df: pd.DataFrame):
    ...
    def get_mismatched_atoms(self, row: dict[str, str]):
        stereo_difference = row['stereo_difference']
        num_unassigned_stereo_atoms = row['num_unassigned_stereo_atoms']
        return self.sc.get_mismatched_atoms_one_reaction(stereo_difference, num_unassigned_stereo_atoms)

    def get_unassigned_stereo_atoms(self, row: dict[str, str]):
    ...
    ###########################
    def calculate_atom_mapped_reaction_data(self, reaction_smiles_df: pd.DataFrame):
